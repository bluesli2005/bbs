...
class main extends AWS_CONTROLLER
{
	...
	public function setup()
	{
		if (is_mobile() AND !$_GET['ignore_ua_check'])
		{
			switch ($_GET['app'])
			{
				default:
                    if($_GET['category']){
                        HTTP::redirect('/m/explore/category-' . $_GET['category']);
                    }else{
                        HTTP::redirect('/m/');
                    }
				break;
	...
	public function index_action()
	{
		...
		if($_GET['is_reward']){
			$posts_list=$this->model('reward')->get_finance(intval($_GET['page']));
			foreach ($posts_list as $key => $value) {
				$posts_list[$key]['user_info']=$this->model('account')->get_user_info_by_uid($value['published_uid']);
			}
			TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
				'base_url' => get_js_url('/is_reward-1'),
				'total_rows' => $this->model('reward')->get_reward_total(),

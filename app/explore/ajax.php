	...
	public function list_action()
	{
		...
		if (is_mobile()  AND $posts_list)
		{
			foreach ($posts_list AS $key => $val)
			{
				...
				{
					case 'question':
	                    $posts_list[$key]['question_detail'] = cjk_substr(preg_replace('/\[attach\]([0-9]+)\[\/attach]/', '', strip_tags(html_entity_decode(FORMAT::parse_attachs(FORMAT::parse_bbcode($posts_list[$key]['question_detail']))))), 0, 100, 'UTF-8', '...');
						break;

					case 'article':
	                    $posts_list[$key]['message'] = cjk_substr(preg_replace('/\[attach\]([0-9]+)\[\/attach]/', '', strip_tags(html_entity_decode(FORMAT::parse_attachs(FORMAT::parse_bbcode($posts_list[$key]['message']))))), 0, 150, 'UTF-8', '...');
						break;
				}
			}
		}

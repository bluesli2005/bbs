...
class main extends AWS_CONTROLLER
{
	...
	public function setup()
	{
        ...
        // if ($this->db_version == 20180601 OR $this->db_version == 20180503)
        // {
        // }
            // $this->db_version = end($this->versions);
	...
	public function index_action()
	{
		...
		TPL::assign('versions', end($this->versions));
	...
	public function run_action(){
		...
		$key=array_search($this->db_version,$this->versions);
		$db=array_slice($this->versions, $key+1);
	
		foreach ($db as $k => $v) {
			$sql_query = null;
			$sql_file = ROOT_PATH . 'app/upgrade/db/' . $v . '.sql';
			$upgrade_script = ROOT_PATH . 'app/upgrade/script/' . $v . '.php';
			if ($this->db_version <= $v AND file_exists($sql_file))
			{
				$sql_query = file_get_contents($sql_file);
			}
			if ($this->db_version == $v AND $this->ignore_sql)
			{
				unset($sql_query);
			}
			if (trim($sql_query))

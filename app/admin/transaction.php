...
class transaction extends AWS_ADMIN_CONTROLLER
{
	...
	public function apply_action(){
		TPL::assign('menu_list', $this->fetch_menu_list());
		if ($apply_list1 = $this->model('account')->fetch_page('user_withdraw', '`status`=0', 'id DESC', intval($_GET['page']), $this->per_page))
		{
			$apply_total1 = $this->model('account')->found_rows();
		}
		if ($apply_list2 = $this->model('account')->fetch_page('user_withdraw', '`status`=1', 'id DESC', intval($_GET['page']), $this->per_page))
		{
			$apply_total2 = $this->model('account')->found_rows();
		}
		if ($apply_list3 = $this->model('account')->fetch_page('user_withdraw', '`status`=2', 'id DESC', intval($_GET['page']), $this->per_page))
		{
			$apply_total3 = $this->model('account')->found_rows();
		}
		if ($apply_list4 = $this->model('account')->fetch_page('user_withdraw', '`status`=3', 'id DESC', intval($_GET['page']), $this->per_page))
		{
			$apply_total4 = $this->model('account')->found_rows();
		}
		TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
	...
	public function previews_action(){
		$id=intval($_GET['id']);
		if ($apply = $this->model('account')->fetch_row('user_withdraw',"`id`=".$id."" ))
	...
	public function export_excel_action(){
        ...
        $list=$this->model('account')->fetch_all('user_withdraw',"status=0 and id in ($ids)");           
		$this->model('account')->update('user_withdraw',array('status'=>1),"id in ($ids)");
		$this->model('excel')->export($list);
	}
	public function carry_action(){
		$id=intval($_GET['id']);
		$this->model('account')->update('user_withdraw',array('status'=>3),'id='.$id);
		$mantotal=$this->model('account')->fetch_one('sysaccount','total','','id desc');
		$user_withdraw=$this->model('account')->fetch_row('user_withdraw','id='.$id);
		$this->model('account')->insert('sysaccount',array(

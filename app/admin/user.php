...
class user extends AWS_ADMIN_CONTROLLER
{
    public function list_action()
    {
        ...
        if ($_GET['reg_time'] == 'down')
        {
            $order = "reg_time ASC";
        }
        if ($_GET['last_active'] == 'up')
        {
            $order = "last_active DESC";
        }
        if ($_GET['last_active'] == 'down')
        {
            $order = "last_active ASC";
        }
        if ($_GET['online_time'] == 'up')
        {
            $order = "online_time DESC";
        }
        if ($_GET['online_time'] == 'down')
        {
            $order = "online_time ASC";
        }
        if ($_GET['reputation'] == 'up')
        {
            $order = "reputation DESC";
        }
        if ($_GET['reputation'] == 'down')
        {
        ...
        if ($_GET['integral'] == 'down')
        {
            $order = "integral ASC";
        }
        
        $user_list = $this->model('people')->fetch_page('users', implode(' AND ', $where), $order, $_GET['page'], $this->per_page);
        ...
        $resons = get_setting('del_reason')?get_setting('del_reason'):[];
        $resons = str_replace(array("\r\n", "\r", "\n"), PHP_EOL, $resons);
        $resons = explode(PHP_EOL,$resons);
        TPL::assign('reason_list', $resons);
     ...
     * @param $where
     * @author Laushow
     * @datatime 2018/7/19 9:32
     */
    private function remove_user($where=array())
    {
        $where[] = 'is_del = 1';
        ...
        TPL::assign('remove_list', $user_list);
    }
}
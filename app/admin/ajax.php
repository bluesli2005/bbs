...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    public function login_process_action()
    {
        ...
        if (get_setting('admin_login_seccode') == 'Y' AND !$this->model('tools')->geetest($_POST))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请填写正确的验证码')));
        }
    ...
    public function save_settings_action()
    {
        ...
        if ($_POST['report_reason'])
        {
            ...
            $_POST['report_reason'] = implode(',',(preg_split("/\n/",$_POST['report_reason'])));
        }
    ...
    {
        if (!in_array($_POST['batch_type'], array('approval', 'decline','remove')))
        {
    ...
    public function article_manage_action()
    {
        ...
        switch ($_POST['action'])
        {
            ...
            case 'recover':
                foreach ($_POST['article_ids'] AS $article_id)
                {
                    $this->model('article')->recover_question($article_id);
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
    ...
    public function nav_menu_upload_action()
    {
        ...
        AWS_APP::upload()->initialize(array(
            ...
            'encrypt_name' => FALSE
        ))->do_upload('aws_upload_file'.intval($_GET['id']));
    ...
    public function question_manage_action()
    {
        ...
        switch ($_POST['action'])
        {
            ...
            case 'recover':
                foreach ($_POST['question_ids'] AS $question_id)
                {
                    $this->model('question')->recover_question($question_id);
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
        ...
        if ($_POST['email_list'])
        {
            if ($emails = explode("\n", str_replace("\r", "\n", $_POST['email_list'])))
            {
                foreach($emails as $key => $email)
                {
                    ...
                    {
                        H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t($email.'该邮箱已注册，不能邀请')));
                    }
    ...
    public function remove_users_action()
    {
        ...
        if(!$_POST['reason']){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择删除原因')));
        }
        ...
        foreach ($_POST['uids'] AS $uid)
        {
            ...
            if ($user_info)
            {
                ...
                $this->model('system')->remove_user_by_uid($uid, $_POST['reason']);
            }
            else
            {
                continue;
     ...
     * @description [删除用户恢复]
     * @author Laushow
     * @datatime 2018/7/19 9:29
     */
    public function remove_recover_action()
    {
        if (!is_array($_POST['uids']) OR !$_POST['uids']) {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择要恢复的用户')));
        }
        @set_time_limit(0);
        foreach ($_POST['uids'] AS $uid) {
            $user_info = $this->model('account')->get_user_info_by_uid($uid,false,true,false);
            if ($user_info) {
                $this->model('system')->recover_user_by_uid($uid);
            } else {
                continue;
            }
        }
     ...
     * @description [文章评论]
     * @author Laushow
     * @datatime 2018/7/19 11:54
     */
    public function article_comment_manage_action(){
        if (!$_POST['ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择评论进行操作')));
        }
        ...
        switch ($_POST['action'])
        {
            case 'del':
                ...
                {
                    $this->model('article')->remove_comment($comment_id);
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
                break;
            case 'recover':
                foreach ($_POST['ids'] AS $comment_id)
                {
                    $this->model('article')->recover_comment($comment_id);
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
                break;
        }
     ...
     * @description [添加广告]
     * @author Laushow
     * @datatime 2018/9/27 14:05
     */
    public function edit_add_action()
    {
        $data['type'] = $_POST['type']?$_POST['type']:0;
        $data['title'] = $_POST['title']?strip_tags($_POST['title']):'';
        $data['link'] = $_POST['link']?strip_tags($_POST['link']):'';
        $data['open_type'] = $_POST['open_type']?intval($_POST['open_type']):1;
        $data['status'] = $_POST['status']?intval($_POST['status']):2;
        $data['type'] = $_POST['type']?intval($_POST['type']):1;
        $ad_id = $_POST['ad_id']?intval($_POST['ad_id']):0;
        if($data['type'] == 3){
            $data['content'] = $_POST['content']?$_POST['content']:'';
            $data['pic'] = '';
            if(empty($data['content'])){
                H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('广告内容不能为空.')));
            }
        } else {
            if ($ad_id <= 0) {
                if (empty($_FILES['pic'])) {
                    H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('广告图片不能为空.')));
                }
            }
            if ($_FILES['pic']['name']) {
                AWS_APP::upload()->initialize(array(
                    'allowed_types' => 'jpg,jpeg,png,gif',
                    'upload_path' => get_setting('upload_dir') . '/ad',
                    'is_image' => TRUE
                ))->do_upload('pic');
                if (AWS_APP::upload()->get_error()) {
                    switch (AWS_APP::upload()->get_error()) {
                ...
                $data['content'] = '';
                $data['pic'] = get_setting('upload_url') . '/ad/'.$upload_data['file_name'];
            }
        }
        if ($data['type'] <= 0) {
            H::ajax_json_output(AWS_APP::RSM(null, -1, '广告类型错误，请稍后重试.'));
        } else if(empty($data['title'])){
            H::ajax_json_output(AWS_APP::RSM(null, -1, '广告标题不能为空.'));
        } else if(empty($data['link'])){
            H::ajax_json_output(AWS_APP::RSM(null, -1, '广告连接不能为空.'));
        }
        if ($data['type'] == 1) {
            $count = $this->model('ad')->count('ad', 'type=' . $data['type']);
            if ($count >= 3 && $ad_id <= 0) {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '不能超过3张广告.'));
            }
        } else {
            $info = $this->model('ad')->fetch_row('ad', 'type=' . $data['type']);
            if ($info && $info['id'] != $ad_id) {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '除首页外同一广告位只能添加一张广告.'));
            }
        }
        if($ad_id <= 0){
            $this->model('ad')->add_ad($data);
        } else {
            $this->model('ad')->edit_ad($data,$ad_id);
        }
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }
     ...
     * @description [修改广告状态]
     * @author Laushow
     * @datatime 2018/9/28 9:07
     */
    public function set_ad_status_action(){
        $id = intval($_POST['id']);
        ...
        $this->model('ad')->edit_ad(['status'=>$status],$id);
        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }
     ...
     * @description [删除广告]
     * @author Laushow
     * @datatime 2018/9/28 14:30
     */
    public function del_ad_action(){
        if(intval($_POST['id']) > 0){
            $this->model('ad')->delete('ad', 'id=' . intval($_POST['id']));
            H::ajax_json_output(AWS_APP::RSM(null, 1, null));
        } else {
     ...
     * @description [封禁IP]
     * @author Laushow
     * @datatime 2018/10/9 15:49
     */
    public function ban_ip_action()
    {
        $ip = $_POST['ip'];
        $type = $_POST['type'];
        if ($type == 1) { #封禁
            if ($this->model('banip')->check_ip($ip)) {
                H::ajax_json_output(AWS_APP::RSM(null, 2, '当前IP已被封禁.'));
            } else if ($this->model('banip')->ban_ip($this->user_id, $ip)) {
                H::ajax_json_output(AWS_APP::RSM(null, 1, 'IP封禁成功.'));
            } else {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '封禁失败,请稍后重试.'));
            }
        } else if ($type == 2) {#解封
            if ($this->model('banip')->check_ip($ip)) {
                if ($this->model('banip')->unban_ip($this->user_id, $ip)) {
                    H::ajax_json_output(AWS_APP::RSM(null, 1, 'IP解封成功.'));
                } else {
                    H::ajax_json_output(AWS_APP::RSM(null, 2, 'IP解封失败.'));
                }
            } else {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '当前IP未被封禁.'));
            }
        }
    }
     ...
     * @description [检测IP]
     * @author Laushow
     * @datatime 2018/10/9 16:43
     */
    public function check_ip_action(){
        $ip = $_GET['ip'];
        if ($this->model('banip')->check_ip($ip) === true) {
    ...
    public function apply_handle_action(){
        ...
        // var_dump($_POST);die;
        $table=$type=='appeal'?'answer_appeal':'user_withdraw';
        $ret=$this->model('reward')->update($table,$data,'id='.$data['id']);
        if($ret!==false)
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('审核完成')));
    }
    //提现审核
    public function save_withdraw_action()
    {       
        if (!$withdraw = $this->model('withdraw')->get_withdraw_info_by_id($_POST['id'])){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('该条提现信息不存在')));
        }
        ...
        if ($_POST['status'] == 2 && empty($_POST['remarks'])){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请填写备注')));
        }

        $re = $this->model('withdraw')->update_withdraw_attrib_fields(array(
            'status' => intval($_POST['status']),
            'remarks' => htmlspecialchars($_POST['remarks']),
        ), $withdraw['id']);

        if($re)
            H::ajax_json_output(AWS_APP::RSM(null, 1, AWS_APP::lang()->_t('审核成功')));   
        else
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('审核失败')));  
           
    }
    public function save_reward_setting_action(){
     if($_POST['tip_enabled']=='Y'){
            if(trim($_POST['tip_money'])){
                $_POST['tip_money'] = trim($_POST['tip_money']);
                $_POST['tip_money'] = implode(',',(preg_split("/\n/",$_POST['tip_money'])));
            }
        }

        if($_POST['reward_enabled']=='Y'){
            if(intval($_POST['longest_period'])<=0)
             H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('最长有效期必填且必须大于0')));
        }
        if($_POST['cash_method']==1){
            if(isset($_POST['cash_date']) && intval($_POST['cash_date'])<=0){
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('每月提现日必须为数字')));
            }
            if(isset($_POST['cash_date']) && intval($_POST['cash_date'])>31){
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('每月提现日不能大于31')));
            }
            
        }else{
            if(isset($_POST['cash_times_month']) && intval($_POST['cash_times_month'])<=0){
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('每月提现次数必填且必须为数字')));
            }
            
        }
        if(isset($_POST['cash_max']) && intval($_POST['cash_max'])<=0){
             H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('每次提现最大限额必填且必须为数字')));
        }
        if(isset($_POST['cash_min']) && intval($_POST['cash_min'])<=0){
             H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('每次提现最小限额必填且必须为数字')));
        }
        if(isset($_POST['publicity']) && intval($_POST['publicity'])<=0){
             H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('悬赏公示期必须是数字')));
        }
            if(isset($_POST['publicity']) && intval($_POST['publicity'])>31){
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('悬赏公示期不能大于31')));
            }
        if(count($_POST['pay_method'])>0){
            if(in_array(1,$_POST['pay_method'])){
                if(!$_POST['reward_wx_appid'])
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('公众号APPID必填')));
                // if(!$_POST['weixin_app_secret'])
                //  H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('公众号APPSECRET必填')));
                if(!$_POST['reward_wx_mchid'])
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('微信商户号必填')));
                 if(!$_POST['reward_wx_key'])
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('微信KEY商户密匙必填')));
             }
            if(in_array(2,$_POST['pay_method'])){
                if(!$_POST['reward_ali_appid'])
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('支付宝app_id必填')));
                if(!$_POST['reward_ali_private_key'])
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('支付宝商户私钥必填')));
                if(!$_POST['reward_ali_public_key'])
                 H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('支付宝商户公钥必填')));
             }
        }else{
             $_POST['pay_method']='';
        }

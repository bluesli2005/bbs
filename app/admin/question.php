...
class question extends AWS_ADMIN_CONTROLLER
{
	...
	public function question_list_action()
	{
		...
		$this->question_remove_action($where);
        $where[] = 'is_del = 0';
        if($_GET['cur'] == 'list'){
            $pages = $_GET['page'];
        } else {
		...
		if ($question_list = $this->model('question')->fetch_page('question', implode(' AND ', $where), 'question_id DESC', $pages, $this->per_page))
		{
     ...
     * @description [已删除问题]
     * @author Laushow
     * @datatime 2018/7/18 10:03
     */
    private function question_remove_action($where)
    {
        $where[] = 'is_del = 1';
        ...
        if ($question_list = $this->model('question')->fetch_page('question', implode(' AND ', $where), 'question_id DESC', $page, $this->per_page)) {
            $total_rows = $this->model('question')->found_rows();
        ...
        TPL::assign('question_remove_count', $total_rows);
        TPL::assign('remove_list', $question_list);
    }
}
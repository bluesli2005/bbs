...
class ajax extends AWS_CONTROLLER
{
	...
	public function fetch_answer_data_action()
	{
		...
		$answer_info['answer_content']=html_entity_decode($answer_info['answer_content']);
	...
	public function get_answer_comments_action()
	{
		...
		{
			TPL::output("question/ajax/answer_comments");
		}
	}
	...
	public function save_answer_action()
	{
		...
		if (!is_mobile() && human_valid('answer_valid_hour') AND !$this->model('tools')->geetest($_POST)) {
		...
		else
		{
			...
			if ($answer_info['has_attach'])
			{
                ...
                foreach ($answer_info['attachs'] as $key=>$val){
                    if(strpos($answer_info['answer_content'],$val['file_location']) !== false){
                        $answer_info['insert_attach_ids'][] = $val['id'];
                    }
                }
			}

			$answer_info['user_info'] = $this->user_info;
			$answer_info['answer_content'] = html_entity_decode($this->model('question')->parse_at_user(FORMAT::parse_attachs((FORMAT::parse_bbcode($answer_info['answer_content'])))));
	...
	public function remove_comment_action()
	{
		...
		if ($_GET['type'] == 'answer')
		{
			$this->model('answer')->save_answer($comment['answer_id']);
		}
		...
		H::ajax_json_output(AWS_APP::RSM(array('answer_id'=>$comment['answer_id']), 1, null));
	}
	...
	public function save_report_action()
	{
		...
		$url = $_POST['type'] == 'question' || $_POST['type'] == 'article' ? $_SERVER['HTTP_REFERER'] : $_SERVER['HTTP_REFERER'].'#!answer_'.$_POST['target_id'];
		...
		H::ajax_json_output(AWS_APP::RSM(array('url'=>''), 1, AWS_APP::lang()->_t('举报成功')));
	}
	public function one_best_answer_action()
	{
		...
		if ($answer_info['uid']==$this->user_id and $_POST['type']==2)
		{
		...
		if ($answer_info['uid']==$question_info['published_uid'] and $_POST['type']==2)
		{
	...
	public function set_best_answer_action()
	{
		...
		$ubest_count=$this->model('answer')->count('answer','is_best=1 and question_id='.$question_info['question_id'].' and uid='.$answer_info['uid']);
		if($ubest_count>0 and $_POST['type']==2){
			H::ajax_json_output(AWS_APP::RSM(null, - 1, AWS_APP::lang()->_t('同一个回复者的多个回答只能设置一个最佳回复')));
		}
		$best_count=$this->model('answer')->count('answer','is_best=1 and question_id='.$question_info['question_id']);
		if ($best_count>=3 and $_POST['type']==2)
		{
			H::ajax_json_output(AWS_APP::RSM(null, - 1, AWS_APP::lang()->_t('最佳回复不能超过三个')));
		}
		// if ($question_info['best_answer'])
		// {
		// 	H::ajax_json_output(AWS_APP::RSM(null, - 1, AWS_APP::lang()->_t('问题已经存在最佳回复')));
		// }
		// var_dump($best_count==2 and $_POST['type']==2);die;
		$this->model('answer')->set_best_answer($_POST['answer_id'],$this->user_id,$_POST['type']);
		if($this->user_info['permission']['is_moderator'] OR $this->user_info['permission']['is_administortar']){
			$arr=['best_time'=>$question_info['best_time']];
		}else{
			$time=$_POST['type']==2?time():$question_info['best_time'];

			$arr=['best_time'=>$time];

		}
		$this->model('answer')->update('question',$arr,'question_id='.$question_info['question_id']);
		if($best_count==2 and $_POST['type']==2){
			$this->model('answer')->update('question',array('is_reward'=>2,'best_time'=>time()),'question_id='.$question_info['question_id']);
			// $this->model('email')->send($value['uid'],'您的参与有最佳回复','您参与的问题已经最佳答案，快去看看吧', get_js_url('/question/'.$value['question_id'] ),$value['question_content']);

		}
		if($_POST['type']==1)
		H::ajax_json_output(AWS_APP::RSM(null, 1, null));
		else
		H::ajax_json_output(AWS_APP::RSM(array('best_count'=>$best_count,'question_id'=>$question_info['question_id'],'total'=>$question_info['answer_count']), 2, null));
	}
	public function appeal_action(){
		$data['question_id']=intval($_POST['item_id']);
		$data['reason']=htmlspecialchars($_POST['reason']);
		$data['addtime']=time();
		$data['status']=0;
		$data['uid']=$this->user_id;
		if($answer_appeal = $this->model('reward')->fetch_row('answer_appeal','uid='.$this->user_id.' and question_id='.intval($_POST['item_id'])))
			H::ajax_json_output(AWS_APP::RSM(null, 1, AWS_APP::lang()->_t('您已申诉过，请耐心等待处理')));

		$question_info = $this->model('question')->get_question_info_by_id(intval($_POST['item_id']));
		$time=$question_info['best_time']?$question_info['best_time']:$question_info['reward_time'];
		$answer=$this->model('answer')->fetch_row('answer','uid ='.$this->user_id.' and question_id='.intval($_POST['item_id']).' and add_time <'.$time);
		// var_dump($answer);die;
		if(!$answer)
			H::ajax_json_output(AWS_APP::RSM(null, 1, AWS_APP::lang()->_t('您当前不能申诉')));

		if(empty($data['reason'])){
			H::ajax_json_output(AWS_APP::RSM(null, 1, AWS_APP::lang()->_t('申诉理由不能为空')));
		}
		$ret=$this->model('answer')->insert('answer_appeal',$data);
		if($ret){
			H::ajax_json_output(AWS_APP::RSM(null, 1, AWS_APP::lang()->_t('申诉成功，请耐心等待管理员处理')));
		}
	}

	public function over_question_action(){
		$question_id=intval($_POST['question_id']);
		$info=$this->model('reward')->get_user_finance($question_id);
		$this->model('reward')->top_three_money($question_id,$info['money']);
		$this->model('reward')->update('answer_appeal',array('status'=>1),'question_id='.$question_id);
			H::ajax_json_output(AWS_APP::RSM(null, 1, null));

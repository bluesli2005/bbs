	...
	public function index_action()
	{
		...
		if ($_GET['column'] != 'log')
		{
			...
			foreach ($answer_list as $answer)
			{
				...
				$answer['answer_content'] = html_entity_decode($this->model('question')->parse_at_user(FORMAT::parse_attachs((FORMAT::parse_bbcode($answer['answer_content'])))));

				$answer['agree_users'] = $answer_agree_users[$answer['answer_id']];
				$answer['agree_status'] = $answer_vote_status[$answer['answer_id']];
				if(get_setting('tip_enabled')=='Y'){	
					$answer['money'] = $this->model('reward')->get_money($answer['answer_id'],1);
				}
        ...
        $question_info['question_detail'] = htmlspecialchars_decode(FORMAT::parse_attachs((FORMAT::parse_bbcode($question_info['question_detail']))));

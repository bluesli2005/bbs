		...
		if ($answers)
		{
			...
			foreach ($answers AS $key => $val)
			{
				$output['answers'][$val['answer_id']] = array(
					...
					'answer_content' => html_entity_decode(FORMAT::parse_attachs((FORMAT::parse_bbcode($val['answer_content'])))),
			...
			foreach ($questions_info AS $key => $val)
			{
				$output['questions'][$val['question_id']] = array(
					...
					'question_detail' => html_entity_decode(FORMAT::parse_attachs((FORMAT::parse_bbcode($val['question_detail'])))),

...
class main extends AWS_CONTROLLER
{
	...
	public function question_action()
	{
		...
		$question_info['question_detail'] = html_entity_decode(FORMAT::parse_attachs((FORMAT::parse_bbcode($question_info['question_detail']))));
	...
	public function index_action()
	{
		...
		$this->crumb(get_setting('site_name'), '/m/');
	...
	{
        if($_GET['category']){
            HTTP::redirect('/m/category-' . $_GET['category']);
        }else{
            HTTP::redirect('/m/');
        }
	}
	...
	public function people_action()
	{
		...
		else
		{
			...
			// {
			// 	HTTP::redirect('/m/people/' . $user['user_name']);
			// }
		...
		$user_actions_questions=$this->model('actions')->get_user_actions($user['uid'], get_setting('contents_per_page'), ACTION_LOG::ADD_QUESTION, $this->user_id);
		$user_actions_answers= $this->model('actions')->get_user_actions($user['uid'], get_setting('contents_per_page'), ACTION_LOG::ANSWER_QUESTION, $this->user_id);
		TPL::assign('user_actions_questions',$user_actions_questions);
		TPL::assign('user_actions_answers',$user_actions_answers);
		// TPL::assign('user_article_count',$this->user['article_count']?$this->user['article_count']:0);
		TPL::assign('user_actions_questions_count', $user_actions_questions==false?0:count($user_actions_questions));
		TPL::assign('user_actions_answers_count', $user_actions_answers==false?0:count($user_actions_answers));
		TPL::assign('user_actions_articles_count', count($this->model('actions')->get_user_actions($user['uid'],null, ACTION_LOG::ADD_ARTICLE, $this->user_id)));
	...
	public function topic_action()
	{
		...
		// {
		// 	HTTP::redirect('/m/topic/' . $topic_info['topic_id'] . '?rf=' . $_GET['rf']);
		// }
	...
	public function publish_action()
	{
        ...
        $question_info['question_detail'] = html_entity_decode($question_info['question_detail']);
		TPL::assign('question_info', $question_info);
...
//			$article_info['attachs_ids'] = FORMAT::parse_attachs($article_info['message'], true);
		...
		$article_info['message'] = html_entity_decode(FORMAT::parse_attachs((FORMAT::parse_bbcode($article_info['message']))));
		...
		$is_favorite = $this->model('favorite')->check_favorite($article_info['id'],'article', $this->user_id);
    ...
    public function addarticle_action()
    {
        ...
        if ($_GET['id']) {
            ...
            if (!$this->user_info['permission']['is_administortar'] AND !$this->user_info['permission']['is_moderator'] AND !$this->user_info['permission']['edit_article']) {
                if ($article_info['uid'] != $this->user_id) {
                    H::redirect_msg(AWS_APP::lang()->_t('你没有权限编辑这个问题'), '/m/addarticle/' . $_GET['id']);
                }
            }
            TPL::assign('article_info', $article_info);
        ...
        if (($this->user_info['permission']['is_administortar'] OR $this->user_info['permission']['is_moderator'] OR $article_info['published_uid'] == $this->user_id AND $_GET['id']) OR !$_GET['id']) {
            TPL::assign('attach_access_key', md5($this->user_id . time()));
        }

...
class ajax extends AWS_CONTROLLER
{
    ...
    public function first_find_password_action()
    {
       ...
       if (is_mobile()) {
            ...
            $url = get_js_url('/account/find_password/next_find_password/mobile-'.$_POST['mobile'].'__type-mobile__key-'.$key);
	...
	public function profile_setting_action()
	{
		...
		// 	if ($this->model('account')->check_email($_POST['common_email']))
		// 	{
		// 		H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('邮箱已经存在, 请使用新的邮箱')));
		// 	}

		// 	$update_data['email'] = $_POST['common_email'];

		// 	$this->model('active')->new_valid_email($this->user_id, $_POST['common_email']);
  ...
  //           H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('请输入正确的常用邮箱地址')));
	...
	{
		if ($log = $this->model('integral')->fetch_all('integral_log', 'uid = ' . $this->user_id, 'time DESC,integral ASC', (intval($_GET['page']) * 10) . ', 10'))
		{
	...
	public function verify_action()
	{
		if ($this->is_post() AND !$this->user_info['verified'])
		{
			...
			$this->model('notify')->send(0, $recipient_uid, notify_class::TYPE_REPORT, notify_class::CATEGORY_QUESTION,$verify_id,array(
	...
	public function apply_withdraw_action()
	{
		...
		$fee = get_setting('charges_rate');//手续费比列
		$num = get_setting('cash_times_month');//每周提现次数
		$min_money = get_setting('cash_min');//单次提现最小金额
		$max_money = get_setting('cash_max');//单次提现最大金额
	...
	{   
		...
		H::ajax_json_output(['img_url'=>$url,'order_id'=>$info['order_no'],'check_url'=>'/?/account/ajax/check_status/']);
	}

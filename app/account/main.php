...
class main extends AWS_CONTROLLER
{
   	...
   	public function slide_captcha_action()
    {
        ...
        AWS_APP::session()->gtserver = $status;
        AWS_APP::session()->user_id = $data['user_id'];
        H::ajax_json_output(AWS_APP::RSM($GtSdk->get_response_str(), 1, array('gt_status'=>$status,'gt_user'=>$data['user_id'])));
    }
    ...
    public function validate_slide_captcha_action()
    {
        ...
        if (AWS_APP::session()->gtserver == 1) {   //服务器正常

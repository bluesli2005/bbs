...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		if($this->user_id and get_setting('payment')=='Y'){
			$banlce=$this->model('account')->get_banlce($this->user_id);
			TPL::assign('banlce', $banlce['balance']);
			TPL::assign('haspass', $banlce['deal_pwd']?1:0);
		}
		...
		$this->crumb(AWS_APP::lang()->_t('%s 的个人主页', $user['user_name']), 'people/' . $user['uid']);
		...
		TPL::assign('user_actions_questions_count', count($this->model('actions')->get_user_actions($user['uid'],null, ACTION_LOG::ADD_QUESTION, $this->user_id)));
		TPL::assign('user_actions_answers', $this->model('actions')->get_user_actions($user['uid'], 5, ACTION_LOG::ANSWER_QUESTION, $this->user_id));
		TPL::assign('user_actions_answers_count', count($this->model('actions')->get_user_actions($user['uid'],null, ACTION_LOG::ANSWER_QUESTION, $this->user_id)));
	...
	public function index_square_action()
	{
		...
		else
		{
			...
			TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
				...
				'total_rows' => $this->model('account')->get_user_count(implode(' AND ', $where)) > 100 ? 100 : $this->model('account')->get_user_count(implode(' AND ', $where)),

...
class posts_class extends AWS_MODEL
{
	...
	public function get_hot_posts($post_type, $category_id = 0, $topic_ids = null, $day = 30, $page = 1, $per_page = 10)
	{
		if ($day)
		{
            $add_time = strtotime('-'.($day-1).' day');
            $add_time = strtotime(date('Y-m-d',$add_time));
		}
	...
	public function process_explore_list_data($posts_index)
	{
		...
		if ($question_ids)
		{
			...
			$question_infos = $this->model('question')->get_question_info_by_ids($question_ids,false);
			//判断当前问题是否已被删除
			foreach ($question_infos as $k =>$v)
            {
                if($v['is_del'] == 1)
                {
                    $this->update('posts_index', ['is_del'=>1],'post_id = ' . intval($v['question_id']).' and post_type = "question"');
                }
            }
		}
		...
		if ($article_ids)
		{
            ...
            foreach ($article_infos as $k =>$v)
            {
                if($v['is_del'] == 1)
                {
                    $this->update('posts_index', ['is_del'=>1],'post_id = ' . intval($v['id']).' and post_type = "article"');
                }
            }
		}
		...
		$users_info = $this->model('account')->get_user_info_by_uids($data_list_uids,true);

...
class system_class extends AWS_MODEL
{
	...
	{   
		...
		foreach ($category_list AS $category_id => $val)
		{
			...
			{
				$html .= '<option value="' . $category_id . '" selected="selected">' . $_prefix . AWS_APP::lang()->_t($val['title']) . '</option>';
			}
			else
			{   
				if($is_suggest && preg_match("/([建议]+)/u", $val['title'])){
		    	   ...
		    	   $html .= '<option value="' . $category_id . '">' . $_prefix . AWS_APP::lang()->_t($val['title']) . '</option>';
    ...
    /**
     * @description [旧删除 真实删除]
     * @param $uid
     * @param bool $remove_user_data
     * @return bool
     * @throws Zend_Exception
     * @author Laushow
     * @datatime 2018/7/19 9:14
     */
	public function remove_user_by_uid_phy($uid, $remove_user_data = false)
	{
		$delete_tables = array(
			'active_data',
			'answer_uninterested',
     ...
     * @datatime 2018/7/19 9:14
     */
    public function remove_user_by_uid($uid, $reason)
    {
        #删除用户
        $this->update('users', array('is_del' => '1','reason'=>$reason), 'uid = ' . intval($uid));
        #用户附属表
        $this->update('users_attrib', array('is_del' => '1'), 'uid = ' . intval($uid));
        if ($user_answers = $this->query_all("SELECT answer_id FROM " . get_table('answer') . " WHERE uid = " . intval($uid)))
        {
            foreach ($user_answers AS $key => $val)
            {
                $this->model('answer')->remove_answer($val['answer_id'],2);
                $comment = $this->model('answer')->get_answer_comments($val['answer_id']);
                if($comment){
                    #回答
                    foreach($comment as $c){
                        $this->model('answer')->remove_comment($val['answer_id'],2);
                    }
                }
            }
        }
        ...
        if ($user_articles = $this->query_all("SELECT id FROM " . get_table('article') . " WHERE uid = " . intval($uid)))
        {
            ...
            foreach ($user_articles AS $key => $val)
            {
                $this->model('article')->remove_article($val['id'],2);
            }
        }
        #评论
        if($comment = $this->query_all('select id from '.get_table('article_comments').' where uid ='.intval($uid))){
            foreach ($comment AS $key => $val)
            {
                $this->model('article')->remove_comment($val['id'],2);
            }
        }
        #问题
        if ($user_questions = $this->query_all("SELECT question_id FROM " . get_table('question') . " WHERE published_uid = " . intval($uid)))
        {
            foreach ($user_questions AS $key => $val)
            {
                $this->model('question')->remove_question($val['question_id'],2);
            }
        }
        #问题评论
        if ($comment_questions = $this->query_all("SELECT id FROM " . get_table('question_comments') . " WHERE uid = " . intval($uid)))
        {
            foreach ($comment_questions AS $key => $val)
            {
                $this->model('question')->remove_comment($val['id'],2);
            }
        }

        #通知
        $this->model('notify')->remove_notifications($uid,2);

        #用户关注表
        $this->update('user_follow', array('is_del' => '1'), 'friend_uid = ' . intval($uid).' or fans_uid = '.intval($uid));

        $user_follow = $this->fetch_all('user_follow', "fans_uid = " . intval($uid) . " or friend_uid = ".intval($uid));
        foreach($user_follow as $follow){
            $this->shutdown_update('users', array(
                'fans_count' => $this->count('user_follow', ' is_del = 0 and friend_uid = ' . intval($follow['friend_uid'])),
                'friend_count' => $this->count('user_follow', ' is_del = 0 and fans_uid = ' . intval($follow['friend_uid']))
            ), 'uid = ' . intval($follow['friend_uid']));
        }
     ...
     * @param $uid
     * @author Laushow
     * @datatime 2018/7/19 9:30
     */
    public function recover_user_by_uid($uid){
        $this->update('users', array(
            'is_del' => '0'
        ), 'uid = ' . intval($uid));
        $this->update('users_attrib', array(
            'is_del' => '0'
        ), 'uid = ' . intval($uid));
        if ($user_answers = $this->query_all("SELECT answer_id FROM " . get_table('answer') . " WHERE uid = " . intval($uid)))
        {
            foreach ($user_answers AS $key => $val)
            {
                $this->model('answer')->recover_answer($val['answer_id'],2);
                $comment = $this->model('answer')->get_answer_comments($val['answer_id']);
                if($comment){
                    foreach($comment as $c){
                        $this->model('answer')->recover_comment_answer($val['answer_id'],2);
                    }
                }
            }
        }
        ...
        if ($user_articles = $this->query_all("SELECT id FROM " . get_table('article') . " WHERE uid = " . intval($uid)))
        {
            foreach ($user_articles AS $key => $val)
            {
                $this->model('article')->recover_question($val['id'],2);
            }
        }
        ...
        if($comment = $this->query_all('select id from '.get_table('article_comments').' where uid ='.intval($uid))){
            foreach ($comment AS $key => $val)
            {
                $this->model('article')->recover_comment($val['id'],2);
            }
        }
        ...
        if ($user_questions = $this->query_all("SELECT question_id FROM " . get_table('question') . " WHERE published_uid = " . intval($uid)))
        {
            foreach ($user_questions AS $key => $val)
            {
                $this->model('question')->recover_question($val['question_id'],2);
            }
        }
        ...
        if ($comment_questions = $this->query_all("SELECT id FROM " . get_table('question_comments') . " WHERE uid = " . intval($uid)))
        {
            foreach ($comment_questions AS $key => $val)
            {
                $this->model('question')->recover_comment($val['id'],2);
            }
        }

        #用户关注表
        $this->update('user_follow', array('is_del' => '0'), 'friend_uid = ' . intval($uid).' or fans_uid = '.intval($uid));
        ...
        $this->model('notify')->recover_notifications($uid,2);
        AWS_APP::cache()->clean();
    }
}
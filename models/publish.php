...
class publish_class extends AWS_MODEL
{
	...
	public function publish_answer($question_id, $answer_content, $uid, $anonymous = null, $attach_access_key = null, $auto_focus = true, $reply_to_openid = true)
	{
		...
		if ($reply_to_openid)
		{
			...
			{
				$this->model('edm')->reply_answer_by_email($question_info['question_id'], html_entity_decode(nl2br(FORMAT::parse_bbcode($answer_content))));
			}
		}
	...
	public function publish_question($question_content, $question_detail, $category_id, $uid, $topics = null, $anonymous = null, $attach_access_key = null, $ask_user_id = null, $create_topic = true, $from = null,$is_del)
	{
		if ($question_id = $this->model('question')->save_question($question_content, $question_detail, $uid, $anonymous, null, $from,$is_del))
		{
	...
	public function publish_article_comment($article_id, $message, $uid, $at_uid = null)
	{
		...
		$this->update('article', array(
			'comments' => $this->count('article_comments', 'article_id = ' . intval($article_id).' and is_del=0')
    ...
    public function get_attach_by_ids($ids)
    {
        if ($attachs = $this->fetch_all('attach', 'id in ( ' . implode(',', $ids).')'))
        {
            ...
            foreach ($result AS $key => $val)
            {
                $result[$key] = $this->parse_attach_data($val, $val[0]['item_type'], 'square')[$val[0]["id"]];
            }
    ...
    public function get_attach_where($where)
    {
        return $this->fetch_row('attach',$where);
    }
}

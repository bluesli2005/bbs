...
class answer_class extends AWS_MODEL
{
	...
	public function get_answer_list_by_question_id($question_id, $limit = 20, $where = null, $order = 'answer_id DESC')
	{
		...
		if ($answer_list = $this->fetch_all('answer', 'is_del = 0 and question_id = ' . intval($question_id) . $_where, $order, $limit))
		{
	...
	public function save_answer($question_id, $answer_content, $uid, $anonymous = 0)
	{
		...
		$this->shutdown_update('users', array(
			'answer_count' => $this->count('answer', 'is_del = 0 and uid = ' . intval($uid))
		), 'uid = ' . intval($uid));
     ...
     * @throws Exception
     * @author Laushow
     * @datatime 2018/9/11 11:20
     */
	public function remove_answer_by_id_phy($answer_id)
	{
		if ($answer_info = $this->model('answer')->get_answer_by_id($answer_id))
		{
            ...
            /*更新删除回复首页排序*/
            $this->model('question')->update_index_post_order($answer_info['question_id']);
			$this->model('question')->update_answer_count($answer_info['question_id']);
			$this->model('question')->update_index_post_order($answer_info['question_id']);//删除回复更新首页排序
		}
	...
	{
		return $this->fetch_one('answer', 'answer_id', "is_del = 0 and question_id = " . intval($question_id) . " AND uid = " . intval($uid));
	}
	...
	public function get_last_answer_by_question_ids($question_ids)
	{
		...
		if ($last_answer_query = $this->query_all("SELECT last_answer FROM " . get_table('question') . " WHERE question_id IN (" . implode(',', $question_ids) . ")"))
		{
			foreach ($last_answer_query AS $key => $val)
			{
				if ($val['last_answer'])
				{
					$last_answer_ids[] = trim($val['last_answer'],',');
				}
			}

			if ($last_answer_ids)
			{
				if ($last_answer = $this->fetch_all('answer', "is_del=0 AND answer_id IN (" . implode(',', $last_answer_ids) . ")"))
				{
	...
	{
		$count = $this->count('answer_comments', "is_del = 0 and answer_id = " . intval($answer_id));
	...
	public function insert_answer_comment($answer_id, $uid, $message)
	{
		...
		if ($answer_info['uid'] != $uid)
		{
			$this->model('notify')->send($uid, $answer_info['uid'], notify_class::TYPE_ANSWER_COMMENT, notify_class::CATEGORY_QUESTION, $answer_info['question_id'], array(
				...
				'anonymous' => ($question_info['published_uid'] == $uid && $question_info['anonymous']) || ($answer_info['uid'] == $uid && $answer_info['anonymous']) ? 1 : 0,
			));
	...
	{
		return $this->fetch_all('answer_comments', "is_del = 0 and answer_id = " . intval($answer_id), "time ASC");
	}
	...
	public function remove_comment($comment_id,$whereDel = false)
	{
		...
		$this->update('answer_comments', ['is_del'=>$isdel],"id = " . intval($comment_id).$delWhere);
		$info = $this->get_comment_by_id($comment_id);
        $this->update_answer_comments_count($info['answer_id']);
	}
	...
	public function set_best_answer($answer_id,$uid,$type)
	{
     ...
     * @throws Exception
     * @author Laushow
     * @datatime 2018/7/18 16:19
     */
    public function remove_answer($answer_id,$whereDel = false)
    {
        if ($answer_info = $this->model('answer')->get_answer_by_id($answer_id))
        {
            $delWhere = null;
            ...
            $this->update_answer_comments_count($answer_id);
            $this->update('answer', ['is_del'=>$isdel],"answer_id = " . intval($answer_id).$delWhere);
            $this->model('question')->update_answer_count($answer_info['question_id']);
            $this->model('question')->update_index_post_order($answer_info['question_id']);//更新首页排序
            ACTION_LOG::delete_action_history('associate_type = ' . ACTION_LOG::CATEGORY_ANSWER . ' AND associate_id = ' . intval($answer_id),$whereDel);
            ACTION_LOG::delete_action_history('associate_type = ' . ACTION_LOG::CATEGORY_QUESTION . ' AND associate_action = ' . ACTION_LOG::ANSWER_QUESTION . ' AND associate_attached = ' . intval($answer_id),$whereDel);
            $this->shutdown_update('users', array(
                'answer_count' => $this->count('answer', 'is_del = 0 and uid = ' . intval($answer_info['uid']))
            ), 'uid = ' . intval($answer_info['uid']));
        }
        return true;
    }
     ...
     * @description [恢复回答]
     * @author Laushow
     * $whereDel 是否是删除用户恢复内容
     * @datatime 2018/7/18 17:30
     */
    public function recover_answer($answer_id,$whereDel = false){
        if ($answer_info = $this->model('answer')->get_answer_by_id($answer_id))
        {
            $delWhere = null;
            if($whereDel !== false){
            ...
            $this->update('answer', ['is_del'=>0],"answer_id = " . intval($answer_id)).' '.$delWhere;
            $this->model('question')->update_answer_count($answer_info['question_id']);
            ACTION_LOG::recover_action_history('associate_type = ' . ACTION_LOG::CATEGORY_ANSWER . ' AND associate_id = ' . intval($answer_id),$whereDel);
            ACTION_LOG::recover_action_history('associate_type = ' . ACTION_LOG::CATEGORY_QUESTION . ' AND associate_action = ' . ACTION_LOG::ANSWER_QUESTION . ' AND associate_attached = ' . intval($answer_id),$whereDel);
            $this->shutdown_update('users', array(
                'answer_count' => $this->count('answer', 'is_del = 0 and uid = ' . intval($answer_info['uid']))
            ), 'uid = ' . intval($answer_info['uid']));
        }
     ...
     * @datatime 2018/7/18 17:30
     */
    public function recover_comment_answer($comment_id,$whereDel = false){
        if ($answer_info = $this->get_comment_by_id($comment_id))
        {
            $delWhere = null;
            if($whereDel !== false){
                $delWhere = ' and is_del = '.$whereDel;
            }
            $this->update('answer_comments', ['is_del'=>0],"id = " . intval($comment_id).$delWhere);
            $this->update_answer_comments_count($answer_info['answer_id']);
        }
        return true;
    }
     ...
     * @throws Exception
     * @author Laushow
     * @datatime 2018/9/11 11:21
     */
    public function remove_answer_by_id($answer_id,$whereDel=false)
    {
        if ($answer_info = $this->model('answer')->get_answer_by_id($answer_id))
        {
            $delWhere = null;

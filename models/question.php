...
class question_class extends AWS_MODEL
{
     ...
     * @description [获取问题]
     * @param $question_ids
     * @param bool $isdel 逻辑删除
     * @return bool
     * @author Laushow
     * @datatime 2018/9/10 16:12
     */
	 ...
	 */
	public function save_question($question_content, $question_detail, $published_uid, $anonymous = 0, $ip_address = null, $from = null,$is_del)
	{
		if (!$ip_address)
		{
			$ip_address = fetch_ip();
    ...
    public function remove_question($question_id,$whereDel = false)
    {
        ...
        #更新问题表 状态删除
        $this->update('topic_relation', ['is_del'=>$isdel],"`type` = 'question' AND item_id = " . intval($question_id).$delWhere);
        $this->update('posts_index', ['is_del'=>$isdel],'post_type="question" and post_id = ' . intval($question_id).$delWhere);
        $this->update('question', ['is_del'=>$isdel],'question_id = ' . intval($question_id).$delWhere);
        $this->update('answer', ['is_del'=>$isdel],'question_id = ' . intval($question_id).$delWhere);
        $this->update('posts_index', ['is_del'=>$isdel],'post_id = ' . intval($question_id).' and post_type = "question"'.$delWhere);
        $this->model('topic')->update_discuss_count(2);

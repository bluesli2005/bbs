...
class favorite_class extends AWS_MODEL
{
    ...
    public function check_favorite($item_id, $item_type, $uid)
    {
        if (!$item_id OR !$item_type)
        {
            return false;
        }
        return $this->fetch_one('favorite', 'id', "type = '" . $this->quote($item_type) . "' AND item_id = " . intval($item_id) . ' AND uid = ' . intval($uid));
    }
}
...
class topic_class extends AWS_MODEL
{
	...
	public function get_topic_list_by_js($where = null, $order = 'topic_id DESC', $limit = 100, $page = null,$isd=0,$uid=0)
	{
		...
		if ($topic_list = $this->fetch_page('topic', $where, $order, $page, $limit))
		{
			foreach ($topic_list AS $key => $val)
			{
				$topic_list[$key]['topic_pic'] = get_topic_pic_url('max', $val['topic_pic']);
				$topic_list[$key]['has_focus'] = $this->has_focus_topic($uid,$val['topic_id']);
				$topic_list[$key]['expanded']=true;
				if (!$val['url_token'])
				{
					$topic_list[$key]['url_token'] = rawurlencode($val['topic_title']);
				}
		...
		$topic_list=$isd>0?$topic_list:getpbtns($topic_list);
        return $topic_list;
	...
	public function get_focus_topic_by_js($uid, $limit = 20,$where='',$page)
	{
		if (!$uid)
		{
			return false;
		...
		if ($topic_list = $this->fetch_all('topic', $where.' and topic_id IN(' . implode(',', $topic_ids) . ')', 'add_time desc', $limit,$limit*$page))
		{
			foreach ($topic_list AS $key => $val)
			{
		...
		$count=$this->count('topic', $where.' and topic_id IN(' . implode(',', $topic_ids) . ')');
	...
	public function update_discuss_count($topic_id)
	{
        ...
        #is_del 逻辑删除
		$this->update('topic', array(
			'discuss_count' => $this->count('topic_relation', 'is_del=0 and topic_id = ' . intval($topic_id)),
			'discuss_count_last_week' => $this->count('topic_relation', 'add_time > ' . (time() - 604800) . ' AND topic_id = ' . intval($topic_id).' and is_del=0'),
			'discuss_count_last_month' => $this->count('topic_relation', 'add_time > ' . (time() - 2592000) . ' AND topic_id = ' . intval($topic_id).' and is_del=0'),
			'discuss_count_update' => intval($this->fetch_one('topic_relation', 'add_time', 'topic_id = ' . intval($topic_id).' and is_del=0', 'add_time DESC'))
		), 'topic_id = ' . intval($topic_id));
	}
		...
		if ($uids)
		{
			...
			if ($user_list_query)
			{
				foreach ($user_list_query AS $user_info)
				{
					...
					$user_list[$user_info['uid']]['url'] = get_js_url('/people/' . $user_info['uid']);
				}
			}
		}
	...
	public function analysis_log($log_list)
	{
		...
		foreach ($log_list as $key => $log)
		{
			...
			$user_url = get_js_url('people/' . $user_info_list[$log['uid']]['uid']);

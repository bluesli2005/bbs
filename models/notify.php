...
class notify_class extends AWS_MODEL
{
    ...
    const TYPE_REMOVE_ANSWER_COMMENT	= 112;	// 我发表的回复评论被删除
	...
	const TYPE_REPORT = 150; // 举报
	const TYPE_TICKET_ALLOT = 143; // 工单被分配通知发起人
	const TYPE_TICKET_DEAL = 144; // 工单被分配通知处理人

	const TYPE_CONSULT_PUBLISH = 171; // 发起咨询（to 被咨询者）
    const TYPE_CONSULT_PUBLISH_SELF = 176;//发起咨询（to 用户）
	const TYPE_CONSULT_REPLY = 172;//答复咨询
	const TYPE_CONSULT_REFUSE = 173;//拒绝咨询
	const TYPE_CONSULT_CANCEL = 174;//用户取消咨询
	const TYPE_CONSULT_AUTO_CANCEL = 175;//平台自动取消
    
    const TYPE_MEMBER_SUCCESS = 180; // 开通会员成功

    const TYPE_VERIFY_SUCCESS = 181; // 审核通过
    const TYPE_VERIFY_FAIL = 182; // 审核失败
	...
	public function list_notification($recipient_uid, $read_status = 0, $limit = null)
	{
		...
		foreach ($notify_list as $key => $notify)
		{
			...
			if ($data['from_uid'])
			{
				...
				$tmp_data['p_url'] = get_js_url('/people/' . $user_info['uid']);
			}
			...
			{
				case self::CATEGORY_CONSULT:

					$tmp_data['key_url'] = get_js_url('explore/doact/?p=consult&a=edit&id=' . $data['consult_info']['id']);
					$tmp_data['title'] = $data['consult_info']['title'];
	...
	public function read_notification($notification_id, $uid = null)
	{
		...
		if ($notification_ids)
		{
			...
			$this->update('notification', array(
				'read_flag' => 1
			), 'is_del = 0 and recipient_uid = ' . intval($uid) . ' AND notification_id IN (' . implode(',', $notification_ids) . ')');
	...
	public function mark_read_all($uid)
	{
		$this->update('notification', array(
			'read_flag' => 1
		), 'is_del = 0 and recipient_uid = ' . intval($uid));
			...
			if ($val['extend_count'] > 1)
			{
				...
				foreach($val['extend_details'] AS $action_type => $extend)
				{
					...
					switch ($action_type)
					{
                        ...
                        case self::TYPE_REMOVE_ANSWER_COMMENT:
                            $data[$key]['extend_message'][] = AWS_APP::lang()->_t('%s 个回复评论被删除', $extend['count']);
			...
			else
			{
				switch ($val['action_type'])
				{
					case self::TYPE_PEOPLE_FOCUS:
						...
						$val['title'] = '<a href="' . $val['key_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('关注了你');
                    ...
                    case self::TYPE_REMOVE_ANSWER_COMMENT:
                        $data[$key]['message'] = '<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ' . AWS_APP::lang()->_t('删除了你在问题') . ' <a href="' . $val['key_url'] . '">' . $val['title'] . '</a> ' . AWS_APP::lang()->_t('中的回复评论');
					...
					$data[$key]['message'] = AWS_APP::lang()->_t('%s0 赞了你对文章 %s1', array(
							'<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ',
							('<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'.AWS_APP::lang()->_t('的评论'))
						));	
						break;
					...
					$data[$key]['message'] = AWS_APP::lang()->_t('%s0 赞了你的文章 %s1', array(
						...
						break;
					case self::TYPE_REPORT:
						$data[$key]['message'] = '<a href="' . get_js_url('/admin/question/report_list/') . '">' . $val['title']. '</a>';

						break;
					case  self::TYPE_CONSULT_PUBLISH:
						$url = $val['p_url']?$val['p_url']:'';
						if($url){
							$url = 'href="'.$url.'"';
						}
						$name = $val['p_user_name']?$val['p_user_name']:'匿名用户';
						$data[$key]['message'] = AWS_APP::lang()->_t('%s0 向你发起咨询 %s1', array(
							'<a ' . $url . '>' . $name . '</a> ',
							'<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'
						));
						break;
					case  self::TYPE_CONSULT_PUBLISH_SELF:
						$data[$key]['message'] = AWS_APP::lang()->_t('你向 %s0 发起咨询 %s1', array(
							'<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ',
							'<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'
						));
						break;
					case  self::TYPE_CONSULT_REPLY:
						$data[$key]['message'] = AWS_APP::lang()->_t('%s0 回复了你的咨询 %s1', array(
							'<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ',
							'<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'
						));
						break;
					case self::TYPE_CONSULT_REFUSE:
						$data[$key]['message'] = AWS_APP::lang()->_t('%s0 拒绝了你的咨询 %s1', array(
							'<a href="' . $val['p_url'] . '">' . $val['p_user_name'] . '</a> ',
							'<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'
						));
						break;
					case self::TYPE_CONSULT_CANCEL:
						$url = $val['p_url']?$val['p_url']:'';
						if($url){
							$url = 'href="'.$url.'"';
						}
						$name = $val['p_user_name']?$val['p_user_name']:'匿名用户';
						$data[$key]['message'] = AWS_APP::lang()->_t('%s0 取消了咨询 %s1', array(
							'<a ' . $url . '>' . $name . '</a> ',
							'<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'
						));
						break;
					case self::TYPE_CONSULT_REFUSE:
						$data[$key]['message'] = AWS_APP::lang()->_t('由于长时间未答复，平台自动取消咨询 %s1', array(
							'<a href="' . $val['key_url'] . '">' . $val['title'] . '</a>'
						));
						break;
					case self::TYPE_VERIFY_SUCCESS:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发起的认证申请审核通过，请前往 ').
							'<a href="' . $val['p_url'] . '">个人中心</a> 设置咨询单价';
						break;
					case self::TYPE_VERIFY_FAIL:
						$data[$key]['message'] = AWS_APP::lang()->_t('你发起的认证申请审核失败，原因').'<a>'.$val['title'].'</a>';
						break;
                    case self::TYPE_MEMBER_SUCCESS:
						$data[$key]['message'] = AWS_APP::lang()->_t('你已成功开通会员，请前往 ').
							'<a href="' . $val['p_url'] . '">个人中心</a> 查看会员到期时间';
						break;
				}
			}
			if(!$val['title']){
     ...
     * @description [删除通知]
     * @author Laushow
     * @datatime 2018/7/19 11:01
     */
	public function remove_notifications($uid,$whereDel = false){
        $delWhere = null;
        $isdel = 1;
        if($whereDel !== false){
        ...
        $this->update('notification',['is_del'=>$isdel],'sender_uid='.intval($uid).$delWhere);
    }
     ...
     * @datatime 2018/7/19 11:01
     */
    public function recover_notifications($uid,$whereDel = false){
        $delWhere = null;
        if($whereDel !== false){
            $delWhere = ' and is_del = '.$whereDel;
        }
        $this->update('notification',['is_del'=>0],'sender_uid='.intval($uid).$delWhere);
    }
}

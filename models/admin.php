...
class admin_class extends AWS_MODEL
{
    ...
    {
        $pid=$this->fetch_one('menu','pid','url="'.$select_id.'"');
        $cid=$this->fetch_one('menu','id','url="'.$select_id.'"');
        $admin_menu =$this->fetch_all('menu','status=1');
        $admin_menu=$this::getsub($admin_menu);
        if (!$admin_menu)
        {
            return false;
        }
        // var_dump(stripos($select_id,'p='.$_GET['p'].'&a='.$_GET['a']));
        foreach($admin_menu as $m_id => $menu)
        {
            ...
            if ($menu['children'])
            {
                ...
                {
                    if (stripos($c_menu['url'],'p='.$_GET['p'].'&a='.$_GET['a']))
                    {
                        $admin_menu[$m_id]['children'][$c_id]['select'] = true;
                        $admin_menu[$m_id]['select'] = true;
                    }
                    // var_dump(stripos($c_menu['url'],'p='.$_GET['p'].'&a='.$_GET['a']));
                    if ($cid == $c_menu['id'])
                    {
                        $admin_menu[$m_id]['children'][$c_id]['select'] = true;
                        // $admin_menu[$m_id]['select'] = true;
                    }
                    foreach($c_menu['children'] as $cc_id => $cc_menu)
                    {
                        if(stripos($cc_menu['url'],'p='.$_GET['p'])){
                        $admin_menu[$m_id]['children'][$c_id]['select'] = true;

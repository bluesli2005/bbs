...
class project_class extends AWS_MODEL
{
    ...
    public function get_projects_list($category_id, $approved, $status = null, $page, $per_page, $order_by = 'add_time DESC')
    {
		...
		/* 修改category 的bug 开始 */
		$categorys = $this->query_all("SELECT id FROM " . get_table('category') . " WHERE id =" . intval($category_id)." or parent_id = " . intval($category_id));
        ...
        {
            $where[] = 'category_id IN (' . implode(',', $project_ids) . ')';
        }
        /* 修改category 的bug 结束 */

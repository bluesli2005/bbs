...
class pay_unipay_class extends AWS_MODEL{
  ...
  public function callbalk(){
  		if($this->config['wechat']['app_id'] and $this->config['wechat']['mch_id'] and $this->config['wechat']['mch_key'] and xml_parser(file_get_contents('php://input'))){

      		$verify = $this->get_pay_obj()->driver('wechat')->gateway('scan')->verify(file_get_contents('php://input'));
  		}else{
  			$verify=$_POST;
  		}
      $_POST=$verify;
		...
		$orderinfo=$this->model('reward')->fetch_row('order_detail','no='.$recharge['out_trade_no']);
		if(!$orderinfo){
   		$adddetail=[
  			'title'=>$title.'收入',

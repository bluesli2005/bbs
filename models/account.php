...
class account_class extends AWS_MODEL
{
    ...
    public function get_uids_by_tu_name($name)
    {
        $sql = "select uid from ".$this->get_table('users')." where user_name like '%".$this->quote($name)."%'";
        $uids = $this->query_all($sql);
        $arr = [];
        foreach($uids as $val){
            $arr[] = $val['uid'];
        }
    ...
    public function check_mobile($mobile,$password='')
    {
        if($userinfo=$this->fetch_row('users',"mobile='" . $this->quote($mobile) . "'")){
            if($password){
    ...
    public function login_check_mobile($mobile,$password='')
    {
        if($userinfo=$this->fetch_row('users',"mobile='" . $this->quote($mobile) . "'")){
            if($password){
                if (! $this->check_password($password, $userinfo['password'], $userinfo['salt'])){
    ...
    public function check_user_name($user_name, $password)
    {
        if (!$user_name OR !$password)
        {
            return false;
     ...
     * @return array
     */
    public function check_login_mobile($user_name)
    {
        if (!$user_name)
        {
            return false;
     ...
     * @return array
     */
    public function get_user_attrib_info_by_uid($uid)
    {
        if (! $uid)
        {
            return false;
        }
        return $this->fetch_row('users_attrib', 'uid = ' . intval($uid));
    }
    /**
     * 通过 UID 获取用户信息
     *
     ...
     * @return array
     */
    public function get_user_info_by_uid($uid, $attrib = false, $cache_result = true,$is_del = true)
    {
        if (! $uid)
        {
            return false;
        ...
        if (! $user_info = $this->fetch_row('users', $isdel.'uid = ' . intval($uid)))
        {
            return false;
        }
    ...
    public function get_user_info_by_uids($uids, $attrib = false)
    {
        ...
        if ($user_info = $this->fetch_all('users', "is_del = 0 and uid IN(" . implode(',', $uids) . ")"))
        {
     ...
     * @return int
     */
    public function insert_user($user_name, $password, $email = null, $sex = 0, $mobile = null, $regist_type = 'email')
    {
        if (!$user_name OR !$password)
        {
            return false;
        ...
        $salt = fetch_salt(4);
        $valid_mobile = $regist_type == 'mobile'? 1 : 0;
        ...
        )))
        {
            ...
            $this->insert('user_account', array(
                'uid' => $uid
            ));
     ...
     * @return int
     */
    public function user_register($user_name, $password = null, $email = null, $mobile = null, $regist_type = 'email')
    {
        if ($uid = $this->insert_user($user_name, $password, $email,0,$mobile,$regist_type))
        {
    ...
    {
        return $this->shutdown_update('users', array(
            'notification_unread' => $this->count('notification', 'is_del = 0 and read_flag = 0 AND recipient_uid = ' . intval($uid))
        ), 'uid = ' . intval($uid));
    }
    ...
    public function check_username_char($user_name)
    {
        ...
        $length =  mb_strlen($user_name, 'gb2312');
        // var_dump($length);
        ...
        switch(get_setting('username_rule'))
        {
            ...
            case 1:
                ...
                {
                    return AWS_APP::lang()->_t('请输入 %s 字节的用户名, 允许汉字、字母与数字', ($length_min . ' - ' . $length_max));
                }
            break;
    ...
    public function get_user_attrs($where,$limit,$order){
        $sql="select * from {$this->get_table('users')} as a left join {$this->get_table('users_attrib')} as b on a.uid=b.uid where $where order by $order limit $limit";
        $sql2="select count(*) as num from {$this->get_table('users')} as a left join {$this->get_table('users_attrib')} as b on a.uid=b.uid where $where";
        $data['data']=$this->query_all($sql);
        $data['count']=$this->query_all($sql2)[0]['num'];
        return $data;
    }
    public function get_users_list($where, $limit = 10, $attrib = false, $exclude_self = true, $orderby = 'uid DESC',$attrib_order=null,$attrib_where=null)
    {
        if ($where)
        {
        ...
        if ($result)
        {
            ...
            if ($attrib AND $uids)
            {
                $where2='uid IN(' . implode(',', $uids) . ')';
                if($attrib_where){
                    $where2='uid IN(' . implode(',', $uids) . ') and '.$attrib_where;
                }
                if ($users_attrib = $this->fetch_all('users_attrib',$where2,$attrib_order))
                {
                    foreach ($users_attrib AS $key => $val)
                    {
                        unset($val['id']);
    ...
    public function associate_remote_avatar($uid, $headimgurl)
    {
        ...
        curl_close($ch);
        $downloaded_file = fopen($avatar_location, 'w');
     ...
     * @param  string
     */
    public function update_user_trade($mobile, $password, $uid)
    {
        if (!$uid)
        {
            return false;
        ...
        if ($this->count('users', "uid = " . intval($uid)) != 1)
        {
            return false;
        }
        $salt = fetch_salt(4);
       return $this->update('user_account', array(
            'deal_pwd' => compile_password($password, $salt),
            'deal_salt' => $salt
        ), 'uid = ' . intval($uid));

    }

    public function get_users_uid(){
       $uids= $this->query_all('select uid from aws_users');
       foreach ($uids as $key => $value) {
            $data['uid']=$value['uid'];
            $info=$this->fetch_one('user_account','uid','uid='.$value['uid']);
            if(!$info)
                $ret=$this->insert('user_account',$data);
       }
       return $ret;
    }

    public function get_users_by_counts($uid,$where, $orderby, $limit,$page){
        if ($uid) {
            $ids = $this->model('follow')->get_user_friends_ids($uid);
            if (!$ids)
                $where = 'uid!=' . $uid;
            else
                $where = 'uid!=' . $uid . ' and uid not in (' . implode(',', $ids) . ')';
        } else {
            $where = '';
        }

        $ret = $this->fetch_all('users', $where, $orderby, $limit, $page);
        foreach ($ret as $key => $value) {
            $ret[$key]['attrib']=$this->fetch_row('users_attrib','uid='.$value['uid']);
            $ret[$key]['follow_check'] = $this->model('follow')->user_follow_check($uid, $value['uid']);
        }
        return $ret;
    }

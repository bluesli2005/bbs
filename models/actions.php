...
class actions_class extends AWS_MODEL
{
	...
	public function get_user_actions_count($uid, $actions = false, $this_uid = 0)
	{
		...
		$count = ACTION_LOG::get_action_count(implode($where, ' OR '), $show_anonymous);
	...
	public function get_user_actions($uid, $limit = 10, $actions = false, $this_uid = 0)
	{
		...
		if (!$uid)
		{
			$where[] = "is_del=0 AND (associate_type = " . ACTION_LOG::CATEGORY_QUESTION . " AND associate_action IN(" . $this->quote($associate_action) . "))";
		}
		else
		{
			$where[] = "is_del=0 AND (associate_type = " . ACTION_LOG::CATEGORY_QUESTION . " AND uid = " . intval($uid) . " AND associate_action IN(" . $this->quote($associate_action) . "))";
		}
						...
						default:
							...
							)) AND $question_info['answer_count'])
							{
								if ($answer_list = $this->model('answer')->get_answer_by_id($val['associate_attached']))
								{
									...
									$answer_list['answer_content'] = chinese_msubstr(html_entity_decode(FORMAT::parse_attachs((FORMAT::parse_bbcode($answer_list['answer_content'])))),0,135,'utf-8');

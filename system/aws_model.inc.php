...
class AWS_MODEL
{
	...
	public function join($table,$name_cond=[],$type="left",$column=array('*'),$where = null, $order = null, $limit = null, $offset = 0){
		$this->slave();
		$select = $this->select();
		$select->from($this->get_table($table), $column);
			$type='join'.ucfirst($type);
			foreach ($name_cond as $key => $value) {
				$select->$type($this->get_table($key),$value,$column);
			}
		if ($where)
		{
			$select->where($where);

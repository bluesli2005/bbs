...
class ACTION_LOG
{
	...
	public static function get_action_by_where($where = null, $limit = 20, $show_anonymous = false, $order = 'add_time DESC')
	{
		...
		{
            foreach ($user_action_history AS $key => $val) {
                if($val['associate_action'] == 101 || $val['associate_action'] == 201){
                    switch($val['associate_type']){
                        case 1:
                            $info = AWS_APP::model()->fetch_one('question', 'is_del', 'question_id=' . intval($val['associate_id']));
                            if ($info == 0) {
                                $history_ids[] = $val['history_id'];
                            } else {
                                unset($user_action_history[$key]);
                            }
                            break;
                        case 2:
                            $info = AWS_APP::model()->fetch_one('answer', 'is_del', 'question_id=' . intval($val['associate_id']));
                            if ($info == 0) {
                                $history_ids[] = $val['history_id'];
                            } else {
                                unset($user_action_history[$key]);
	...
	public static function get_action_count($where = null, $show_anonymous = false)
	{   
		if (! $where)
		{
			return false;
		...
		$count = AWS_APP::model()->count('user_action_history', $where);
	...
	public static function get_actions_fresh_by_where($where = null, $limit = 20, $show_anonymous = false)
	{
		...
		if ($action_history = AWS_APP::model()->query_all("SELECT history_id,associate_type,associate_id FROM " . get_table('user_action_history_fresh') . " WHERE " . $where . " ORDER BY add_time DESC", $limit))
		{
			foreach ($action_history as $key => $val)
			{
                switch($val['associate_type']){
                    case 1:#问题
                        $isdels = AWS_APP::model()->query_row('select is_del from '.get_table('question').' where question_id='.$val['associate_id']);
                        if($isdels['is_del'] == 0){
                            $history_ids[] = $val['history_id'];
                        }
                        break;
	...
	public static function format_action_data($action, $uid = 0, $user_name = null, $associate_question_info = null, $associate_topic_info = null)
	{
		...
		if ($associate_topic_info AND $associate_topic_info['topic_id'])
		{
	...
	public static function delete_action_history_old($where)
	{
     ...
     * @param $where
     * @author Laushow
     * @datatime 2018/7/30 17:15
     */
    public static function delete_action_history($where,$whereDel=false)
    {
        if ($action_history = AWS_APP::model()->fetch_all('user_action_history', $where))
        {
            $delWhere = null;
            ...
            foreach ($action_history AS $key => $val)
            {
                AWS_APP::model()->update('user_action_history_data',array('is_del'=>$isdel),'history_id = ' . $val['history_id'].$delWhere);
                AWS_APP::model()->update('user_action_history_fresh',array('is_del'=>$isdel),'history_id = ' . $val['history_id'].$delWhere);
            }
            AWS_APP::model()->update('user_action_history',array('is_del'=>$isdel),$where);
        }
    }
     ...
     * @param $where
     * @author Laushow
     * @datatime 2018/7/30 17:34
     */
    public static function recover_action_history($where,$whereDel = false)
    {
        if ($action_history = AWS_APP::model()->fetch_all('user_action_history', $where))
        {
            $wheres = null;
            if($whereDel !== false){
                $wheres = ' and is_del = '.$whereDel;
                $where.=  ' and is_del = '.$whereDel;
            }
            foreach ($action_history AS $key => $val)
            {
                AWS_APP::model()->update('user_action_history_data',array('is_del'=>0),'history_id = ' . $val['history_id'].$wheres);
                AWS_APP::model()->update('user_action_history_fresh',array('is_del'=>0),'history_id = ' . $val['history_id'].$wheres);
            }
            AWS_APP::model()->update('user_action_history',array('is_del'=>0),$where);
        }
    }

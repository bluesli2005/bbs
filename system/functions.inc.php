...
function date_friendly($timestamp, $time_limit = 604800, $out_format = 'Y-m-d H:i', $formats = null, $time_now = null)
{
	...
	{
		$formats = array(
			'YEAR' => AWS_APP::lang()->_t('%s 年前'),
			'MONTH' => AWS_APP::lang()->_t('%s 月前'),
			'DAY' => AWS_APP::lang()->_t('%s 天前'),
			'HOUR' => AWS_APP::lang()->_t('%s 小时前'),
			'MINUTE' => AWS_APP::lang()->_t('%s 分钟前'),
			'SECOND' => AWS_APP::lang()->_t('%s 秒前'),
			'YEARS' => AWS_APP::lang()->_t('%ss 年前'),
			'MONTHS' => AWS_APP::lang()->_t('%s 月前'),
			'DAYS' => AWS_APP::lang()->_t('%ss 天前'),
			'HOURS' => AWS_APP::lang()->_t('%ss 小时前'),
			'MINUTES' => AWS_APP::lang()->_t('%s 分钟前'),
			'SECONDS' => AWS_APP::lang()->_t('%ss 秒前')
		);
	}
...
{	
	...
	$decrypted = mcrypt_decrypt(MCRYPT_RIJNDAEL_128, G_PRIVATEKEY, $encryptedData, MCRYPT_MODE_CBC, G_IV);

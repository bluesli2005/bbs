...
class core_autoload
{
	...
	private static $aliases = array(
		...
		'PLUTPL'				=> 'class/cls_plugins.inc.php',
    ...
    private static function loader($class_name)
	{
		...
		else
		{
			...
			// 查找 models 目录
			else if (file_exists(ROOT_PATH . 'plugins'.DIRECTORY_SEPARATOR .str_replace(array('_class', ''), array('', ''), 'wc_'.$class_name).DIRECTORY_SEPARATOR.str_replace(array('_class', ''), array('', ''), $class_name). '_model.php'))
			{
				$class_file_location = ROOT_PATH . 'plugins'.DIRECTORY_SEPARATOR .str_replace(array('_class', ''), array('', ''), 'wc_'.$class_name).DIRECTORY_SEPARATOR.str_replace(array('_class', ''), array('', ''), $class_name). '_model.php';
			}
			// 查找 class

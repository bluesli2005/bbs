...
switch ($_POST['step'])
{
	default :
		...
		if (is_really_writable(AWS_PATH . 'config'.DIRECTORY_SEPARATOR) OR defined('IN_SAE'))
		{
			$system_require['config_writable_config'] = TRUE;
		}
		if (is_really_writable(ROOT_PATH . 'cache'.DIRECTORY_SEPARATOR) OR defined('IN_SAE'))
		{
			$system_require['config_writable_cache'] = TRUE;
		}
		if (is_really_writable(ROOT_PATH . 'tmp'.DIRECTORY_SEPARATOR) OR defined('IN_SAE'))
		{
			$system_require['config_writable_tmp'] = TRUE;
		}
		if (is_really_writable(ROOT_PATH . 'uploads'.DIRECTORY_SEPARATOR) OR defined('IN_SAE'))
		{
			$system_require['config_writable_uploads'] = TRUE;
		}
	...
	case 4 :
		...
		$db->insert($db_prefix . 'user_account', array('uid' => 1));
		$db->insert($db_prefix . 'plugins', array('name' =>'editor','title'=>'编辑器富文本','intro'=>'编辑器富文本','author'=>'wecenter','version'=>'1.0.0','state'=>1,'config'=>json_encode(array(
			'type'=>array(
				'title'=>'开启类型',
				'type'=>'radio',
				'value'=>'ckeditor',
        ...
        $insert_query = str_replace(array("\r\n","\r","\n"), "\r", $insert_query);

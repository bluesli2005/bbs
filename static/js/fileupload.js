        ...
        getIframeContentJSON: function (iframe, container) {
            ...
            if (doc.body.innerHTML) {
                ...
                if (this.type == 'file') {
                    ...
                    if (response.error) {
                        if(response.error == '错误代码: upload_file_exceeds_limit'){
                            AWS.alert('文件超出上传限制');
                        }else if(response.error == '错误代码: upload_file_exceeds_form_limit'){
                            AWS.alert('文件超过上传数量限制');
                        }else if(response.error == '错误代码: upload_file_partial'){
                            AWS.alert('上传部分文件');
                        }else if(response.error == '错误代码: upload_no_file_selected'){
                            AWS.alert('没有选择上传的文件');
                        }else if(response.error == '错误代码: upload_no_temp_directory'){
                            AWS.alert('没有临时目录');
                        }else if(response.error == '错误代码: upload_unable_to_write_file'){
                            AWS.alert('上传目录没有写入权限');
                        }else if(response.error == '错误代码: upload_stopped_by_extension'){
                            AWS.alert('该类型文件不应许被上传');
                        }else if(response.error == '错误代码: upload_invalid_filesize'){
                            AWS.alert('文件尺寸过大');
                        }else{
        ...
        render: function (element, json, filesize) {
            if (json) {
                if (!json.error) {
                    ...
                    $(element).detach();
                    if(json.error == '错误代码: upload_file_exceeds_limit'){
                        AWS.alert('文件超出上传限制');
                    }else if(json.error == '错误代码: upload_file_exceeds_form_limit'){
                        AWS.alert('文件超过上传数量限制');
                    }else if(json.error == '错误代码: upload_file_partial'){
                        AWS.alert('上传部分文件');
                    }else if(json.error == '错误代码: upload_no_file_selected'){
                        AWS.alert('没有选择上传的文件');
                    }else if(json.error == '错误代码: upload_no_temp_directory'){
                        AWS.alert('没有临时目录');
                    }else if(json.error == '错误代码: upload_unable_to_write_file'){
                        AWS.alert('上传目录没有写入权限');
                    }else if(json.error == '错误代码: upload_stopped_by_extension'){
                        AWS.alert('该类型文件不应许被上传');
                    }else if(json.error == '错误代码: upload_invalid_filesize'){
                        AWS.alert('文件尺寸过大');
                    }else{

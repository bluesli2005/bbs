$(function()
{
	...
	$('.aw-mod.side-nav a').click(function () {
		if( $(this).attr('rel')=='get_consultation'){
			$("#main_profit").show();
		}else{
			$("#main_profit").hide();
		...
		if ($('#main_title').attr('id') != null && $(this).attr('rel'))
		{
			...
			$('.aw-mod.side-nav a').attr("disabled",true).css("pointer-events","none");  
	...
	$('#bp_more').click(function()
	{
		...
		switch (window.location.hash)
		{
			...
			break;
			case '#focus_column__focus':
				var request_url = G_BASE_URL+'/column/ajax/focus_column_list/page-'+ $(this).attr('data-page');
			break;
			case '#initiated_consultation':
				var request_url = G_BASE_URL+'/home/ajax/consult/type-1__page-'+ $(this).attr('data-page');
			break;
			case '#get_consultation':
				var request_url = G_BASE_URL+'/home/ajax/consult/type-2__page-'+ $(this).attr('data-page');
			break;
			case '#get_ticket':
				var request_url = G_BASE_URL+'/home/ajax/ticket/page-'+ $(this).attr('data-page');
			break;
		}

		$(this).addClass('loading');

		if (window.location.hash == '#focus_topic__focus'||window.location.hash == '#focus_column__focus')
		{
		...
		$.get(request_url+'__random-'+Math.floor(Math.random() * 10000), function (response)
		{
			if ($.trim(response.length))
			{
				...
				var b = document.getElementsByClassName('aw-item');
				if(b.length < G_INDEX_PER_PAGE){
					$(_this).addClass('disabled');
					// $(_this).attr('data-page','');
					$(_this).find('span').html('没有更多了');
				}else{
			...
			$('.aw-mod.side-nav a').removeAttr("disabled").css("pointer-events","auto"); 

$(function()
{
 ...
 //    }
            ...
            if (G_ADVANCED_EDITOR_ENABLE == 'Y'){
                ...
                EDITOR_CALLBACK = function (evt){
                    if (evt.editor.getData().length){
                        $.post(G_BASE_URL + '/account/ajax/save_draft/item_id-'+ITEM_ID+'__type-' +　PUBLISH_TYPE, 'message=' + evt.editor.getData(), function (result) {
                            $('#question_detail_message').html(result.err + ' <a href="#" onclick="$(\'textarea#advanced_editor\').attr(\'value\', \'\'); AWS.User.delete_draft('+ITEM_ID+', \'' + PUBLISH_TYPE + '\'); $(this).parent().html(\' \'); return false;">' + _t('删除草稿') + '</a>');
                        }, 'json');
                    }                    
	                // 自动保存草稿
                ...
                $('#wmd-input').bind('blur',function(){
                    ...
                    if(article_textarea != ''){
                        $.post(G_BASE_URL + '/account/ajax/save_draft/item_id-'+ITEM_ID+'__type-' +　PUBLISH_TYPE, 'message=' + article_textarea, function (result) {
                            $('#question_detail_message').html(result.err + ' <a href="#" onclick="$(\'textarea#advanced_editor\').attr(\'value\', \'\'); AWS.User.delete_draft('+ITEM_ID+', \'' + PUBLISH_TYPE + '\'); $(this).parent().html(\' \'); return false;">' + _t('删除草稿') + '</a>');
                        }, 'json');
                    }
                });
    ...
    $('#saveDraft').click(function() {
        ...
        // alert(article_textarea);
        if(article_textarea != ''){
            $.post(G_BASE_URL + '/account/ajax/save_draft/item_id-'+ITEM_ID+'__type-' +　PUBLISH_TYPE, 'message=' + article_textarea, function (result) {
                if(PUBLISH_TYPE=='article')
                $('#draft_detail_message').html(result.err + ' <a href="#" onclick="$(\'textarea#advanced_editor\').attr(\'value\', \'\'); AWS.User.delete_draft('+ITEM_ID+', \'' + PUBLISH_TYPE + '\'); $(this).parent().html(\' \'); return false;">' + _t('删除草稿') + '</a>');
                else
                $('#question_detail_message').html(result.err + ' <a href="#" onclick="$(\'textarea#advanced_editor\').attr(\'value\', \'\'); AWS.User.delete_draft('+ITEM_ID+', \'' + PUBLISH_TYPE + '\'); $(this).parent().html(\' \'); return false;">' + _t('删除草稿') + '</a>');
            }, 'json');
        }
